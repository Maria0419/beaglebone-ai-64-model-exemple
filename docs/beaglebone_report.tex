...
Use the official BeagleBoard image: \href{https://www.beagleboard.org/distros/bbai64-11-7-2023-09-02-10gb-emmc-ti-edgeai-xfce-flasher}{bbai64-emmc-flasher-debian-11.7-xfce-edgeai-arm64-2023-08-05-10gb.img.xz}.
...
\item Re-export and compile with \texttt{./scripts/tidl82\_export\_compile.sh configs/compile\.yaml}.  This wrapper first exports \texttt{artifacts/model.onnx} from \texttt{artifacts/model.pt} on the host, then compiles that ONNX and writes \path{artifacts/tidl/model-artifacts/square\_seg/}.
\end{enumerate}

For another ONNX model, copy \path{configs/compile_tidl82_template.yaml}, edit the ONNX path, calibration image directory, input size, and output directory, then pass that YAML to the same wrapper.  If \texttt{calibration\_dir} is an absolute host path, the wrapper mounts it read-only into the container automatically.  For the square segmentation model, the one-command wrapper eventually calls the compiler script with the compile preset inside the container:
...
It was generated with the repository \href{https://github.com/Maria0419/Cube-Detector}{github.com/Maria0419/Cube-Detector}.
...
The selected architecture was not the highest floating-point model overall on the GPU.  It was the best small model among the candidates tested through TIDL, based on the mask quality preserved after INT8 quantization on the real board.
...
The final TIDL artifact files are:
...
In this example, graph validation is part of the same export step. Running \texttt{python3 src/export\_onnx.py --config configs/export.yaml} exports the ONNX file, checks that the graph is valid, confirms that the operator set matches this project, runs the CPU ONNX Runtime smoke test, and writes the summary to \texttt{artifacts/onnx\_report.json}.
...
\midrule
\path{configs/train.yaml} & Main training preset. It defines dataset paths, shared training settings, and the \texttt{model:} block used by the example. \\
...
\path{configs/run_cpu_board.yaml} & CPU inference preset for the packaged example layout. The inference script reads the image path, output path, and threshold from this YAML. \\
\path{configs/run_tidl.yaml} & TIDL inference preset for the packaged example on the board. The inference script reads the image path, TIDL artifacts path, and threshold from this YAML. \\
\bottomrule
\end{tabularx}
\end{table}
...
From the project directory, the host-side flow for reproducing the selected model is:
...
Before running the compile step, the user must set the calibration image directory in \path{configs/compile.yaml}. Those images are the representative inputs used for TIDL INT8 calibration.
...
Calibration frames are representative input images used only during TIDL INT8 calibration so the compiler can estimate activation ranges for quantization. They are not labels and they are not used for training.
...
The deploy package generated on the host is:

\begin{lstlisting}
artifacts/square_seg_32x13_tidl82.tar.gz
\end{lstlisting}

Copy that tarball once from the host PC to the BeagleBone AI-64, then unpack it on the board before running the board-side commands:

\begin{lstlisting}[language=bash]
scp artifacts/square_seg_32x13_tidl82.tar.gz debian@192.168.7.2:~
ssh debian@192.168.7.2
rm -rf square_seg_tidl_yaml
mkdir -p square_seg_tidl_yaml
tar -xzf square_seg_32x13_tidl82.tar.gz -C square_seg_tidl_yaml
cd square_seg_tidl_yaml
\end{lstlisting}
...
The local TIDL artifact folder is:

\begin{lstlisting}
artifacts/tidl/model-artifacts/square_seg_32x13
\end{lstlisting}
...
After copying or extracting the package on the board, the short commands inside the deployed package are:

The packaged example includes \path{test_images/img_0001_image.tif} as a sample input, but in normal use the user should edit the \path{image:} field in \path{configs/run_cpu_board.yaml} or \path{configs/run_tidl.yaml} to point to the test image they actually want to run.  The script itself is YAML-driven; for inference it only needs the selected \texttt{--config}.
...
From the host PC, the generated masks can be copied back for inspection with:

\begin{lstlisting}[language=bash]
mkdir -p models/square_seg_32x13/artifacts/board_results
scp debian@192.168.7.2:~/square_seg_tidl_yaml/square_seg_32x13_tidl82/artifacts/cpu_mask.png models/square_seg_32x13/artifacts/board_results/
scp debian@192.168.7.2:~/square_seg_tidl_yaml/square_seg_32x13_tidl82/artifacts/tidl_mask.png models/square_seg_32x13/artifacts/board_results/
\end{lstlisting}
...
\caption{Qualitative comparison of CPU and TIDL outputs for the selected model.}
...
To reproduce the deployed model from scratch:
...
\end{lstlisting}
\item Copy \path{artifacts/square_seg_32x13_tidl82.tar.gz} from the host PC to the board and unpack it there.
\item On the board, run CPU inference from the unpacked example directory:
\begin{lstlisting}[language=bash]
python3 run_tidl.py --config configs/run_cpu_board.yaml
\end{lstlisting}
...
If you are reusing the already released model state and ONNX file, the training step can be skipped and the workflow starts at export or compile, depending on which artifacts you already have.
